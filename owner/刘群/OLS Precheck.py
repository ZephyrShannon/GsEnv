...
def OLS Precheck(X,y):
    dict_precheck = {'OLS summary':ols_summary(X,y), 'VIF':VIF(X),'Condition Number':Con_No(X),\
                     'Breush_Pagan test': Breush_Pagan(X,y),'Goldfeld_Quant test': Goldfeld_Quant(X,y), \
                     'ARCH test': ARCH_test(X,y), 'Breusch Goldfrey test': Breusch_Goldfrey(X,y),\
                     'Durbin Watson test': Durbin_Watson(X,y), 'OLS Influence':OLSinfluence(X,y),\
                     'EllipticEnvelop': EllipticEnvelop(X)
                    }
    return dict_precheck
