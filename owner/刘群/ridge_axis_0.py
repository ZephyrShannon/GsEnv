...
import copy
    ...
    # build regression on time series for each symbol
def ridge_axis_0(context,x,y):
    ...
    # Assign new data
    add_y = pd.DataFrame(np.zeros((len(factor_ls),len(symbol_ls))),columns=symbol_ls)
    add_x = add_y
    
    y_df_new = pd.concat([y_df,add_y])
    
    x_dic_new =copy.deepcopy(x_dic)
    
    # Creat beta DataFrame
    ridge_df = pd.DataFrame(np.zeros((len(symbol_ls),len(factor_ls))),columns=factor_ls)
    
    # Create new factor dictionary
    for key,value in x_dic_new.items():
        new_value = value.asMatrix()
        new_value = pd.concat([new_value, add_x],axis=0)
