...
def apply_to_iterators_separately(apply_func ,*iterators):
    ...
    while available_it.has_next():
        ...
        if skip_row is None:
            ...
            for data in data_list:
                try:
                    one_ret = apply_func(data)
                except Exception as e:
                    reason = 'while applying function[' + apply_func.func_gid+ '] at Key:' + str(available_it.key()) + " error:" + str(e)
                    one_ret = gsUtils.ItContinue(reason)

                if check_for_df:
                    if not isinstance(one_ret, gsUtils.ItContinue):
                        check_for_df = False
                        if isinstance(one_ret, np.ndarray) and isinstance(data, pd.DataFrame)and one_ret.ndim == 2:
    ...
    #                 raise Exception("reform_df:{0}, idx:{1}".format(str(reform_df),str(using_df_idx)))
                if reform_df > 0:
                    if reform_df == reform_all_index_and_columns:
                        one_ret = pd.DataFrame(data=one_ret, index=data.index, columns=data.columns)
                    elif reform_df == reform_index_and_one_columns:
                        one_ret = pd.DataFrame(data=one_ret, index=data.index, columns=['result'])
                    elif reform_df == reform_columns_and_one_index:
                        one_ret = pd.DataFrame(data=one_ret, index=['result'], columns=data.columns)
                    elif reform_df == reform_only_index:
                        one_ret = pd.DataFrame(data=one_ret, index=data.index)
                    else:
                        one_ret = pd.DataFrame(data=one_ret, columns=data.columns)
