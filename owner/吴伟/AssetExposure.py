...
def AssetExposure(context,model,target_dates,asset_weight,factors):
    ...
    factor_exposure = factor_exposure.replace(0, np.nan).fillna(method='ffill').ix[begin_date:end_date]
    factor_exposure = gftIO.convertMatrix2ColumnTab(factor_exposure)
    return gftIO.GftTable(matrix=None, columnTab=factor_exposure,
                          matrixIsFromPython=True, gid='ED1EDCC3F849423EA56889A2B7D2AED5', columnOrders=None)
