...
def Sd(x,maxValue, rmMaxPct, rmMinPct, keepOrder,date):
    ...
    if len(rmx) > 1:
        ...
        if keepOrder == 1:            
            ...
            if len(max_x) >0:
                replace_max_x=sdvalue * maxValue * (1 + max_x.rank(ascending=True,method='average')/len(max_x)/10000)
                result[result > maxValue*sdvalue] = replace_max_x
            
            min_x=result[result < -maxValue*sdvalue]
            if len(min_x) >0:
                replace_min_x=-sdvalue * maxValue * (1 + (abs(min_x)).rank(ascending=True,method='average')/len(min_x)/10000)
                result[result < -maxValue*sdvalue] = replace_min_x      
        else:
            absresult=abs(result)
            result=(np.sign(result)) * (absresult.where(absresult <= maxValue*sdvalue,maxValue*sdvalue))
...
def WinsorizeSd(x,maxValue=5, rmMaxPct=0, rmMinPct=0, keepOrder=0):
    ...
    ls_sd_result=[Sd(x,maxValue, rmMaxPct, rmMinPct, keepOrder,date) for date in ls_date]
    final=pd.concat(ls_sd_result)
