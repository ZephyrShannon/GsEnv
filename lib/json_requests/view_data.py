...
def create_view_data_req(target_gid, begin_date, end_date, file_name, server_gid):
    ...
    if not isinstance(file_name, str):
        file_name = ''
    if not isinstance(server_gid, str):
        server_gid = ''
    return req_str % (target_gid, begin_date, begin_date+2, begin_date, end_date, file_name, server_gid)
...
def get_data_from_gs(gscall, jgid, start_date, end_date, filename=None):
    ...
    raise Exception("Load j from gs failed, server ret is : "+ (str(sever_ret)[:100]))